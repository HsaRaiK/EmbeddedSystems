...
// a decoded string, reads: Atat�rk 1881 - 1938
...
int main(void)
 {
	...
	volatile int * HEX3_HEX0_ptr; // virtual address pointer to right 7-segs
	...
	// goes through the decoded array and prints every single element all the while shifting the previous values to the left by 8 bits (1 display)
	for(int i = 0; i<17; i++){
		*HEX3_HEX0_ptr += decoded[i];
		sleep(1); 
		*HEX3_HEX0_ptr = *HEX3_HEX0_ptr << 8;
	}
	*HEX3_HEX0_ptr = 0; // clear 7-seg display.
