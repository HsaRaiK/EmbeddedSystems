...
irq_handler_t irq_handler(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    if (irq == 72)
    {
        ...
        *(TIMER_ptr + 1) = 0b101;
        ...
        if (time_in <= 0)
            time_in = 360000; // if 0 is reached, wrap around
    ...
    else
    {
        ...
        case 0b0001:
            ...
            if (*(TIMER_ptr + 1) % 2 == 1)
                *(TIMER_ptr + 1) = 0b1000; // stop timer if interrupt is enabled
            else
                *(TIMER_ptr + 1) = 0b0101; // if interrupt is not enabled, continues the timer.
